...
Live coding is an increasingly popular creative practice in which performers write and modify source code in real time to produce music, visuals, or other time-based media\footnote{\href{https://toplap.org/wiki/ManifestoDraft}{TOPLAP Manifesto}}.
...
Coypu\cite{Cipr25c} is a library and domain-specific dialect for live music programming within the Pharo environment. It acts as a client for an audio server, which can be either internal (a Phausto\cite{Cipr24b} DSP running within Pharo) or an external OSC-capable application, typically SuperCollider via the SuperDirt engine, or a MIDI device.
...
\subsection{Method Proxies: dynamic method interception in Pharo}
Method Proxies\cite{Mont24} is a message-passing control library for Pharo that lets you dynamically instrument any method to run user-defined actions before and after its execution. The idea is simple: you tell the framework which method you want to intercept, and you hand it a handler that says what to do around it.

A handler is an instance of a subclass of \texttt{MpHandler}. It defines two hooks: \texttt{beforeExecutionWithReceiver:arguments:}, which fires right before the original method runs, and \texttt{afterExecutionWithReceiver:arguments:returnValue:}, which fires after it returns. That is all the user needs to care about --- everything else is managed under the hood.
Internally, when a method is instrumented, the framework replaces the original compiled method in the method dictionary with a copy of a pre-compiled trap method template. Through literal patching, this trap wires in references to the actual handler and to the original method, which is preserved under a mangled selector. The trap leverages Pharo's stack unwinding mechanism via a marker primitive to guarantee execution of the after-hook without allocating block closures, making it significantly faster than \texttt{ensure:}-based approaches.
Method Proxies also takes care of the tricky parts. A per-thread meta-level detection mechanism prevents infinite recursion when the instrumentation code itself triggers instrumented methods. Thread safety, non-local returns, exceptions, and clean uninstrumentation are all handled by the framework. The entry point is \texttt{MpMethodProxy}, which manages installing and removing the instrumentation.The users just define what happens before and after, and the result is code that is easier to maintain and extend.

\section{Previous Approach: Announcers and Announcements}
The first recording mechanism for Coypu was implemented by Redwane Engels during an internship and demonstrated at ESUG 2024 in Lille. It relied on Pharo's \texttt{Announcer} and \texttt{Announcement} classes, which implement the observer design pattern.
...
\subsection{Timing Reconstruction}
