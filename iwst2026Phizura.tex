...
The extended Playground provides a hook on each evaluated expression before execution. This hook captures the evaluated command together with a timestamp. When recording is active, each evaluation is forwarded to a dedicated handler. The handler creates a structured record containing the evaluated expression and its timestamp, representing an absolute point in time using \texttt{DateAndTime now}. During replay, temporal spacing is reconstructed by computing the difference between consecutive timestamps, yielding a \texttt{Duration} used to reproduce the original timing of the performance.
