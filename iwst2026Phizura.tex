...
%% article.
\begin{abstract}
Live coding is a performance practice in which artists write and modify code in real time to generate music.
\sebas{A word about the artistic part of live coding. It is important to say this because later we say that phizura needs to generate the same code as it was written for artistic reasons.}
A key challenge is capturing a performance for later replay, sharing, or analysis without resorting to audio recording.
\sebas{What is "capturing a performance". It will be better to say that the challenge is to capture idiomatically the evaluations of phizura to later be able to replay the live performance.}
...
The recorded data, timestamped expressions and input events, can later be emitted as executable Pharo code that faithfully replays the performance with the original timing.\sebas{"faithfully" is not the right word for me. we can say that it captures exactly the same evaluated code and later we can export it as pharo pretty code or OSC signals.}
We describe the architecture of Phizura, detail how Method Proxies enable a clean separation between recording logic and the Coypu codebase, and compare the approach to a previous Announcer-based implementation.
\sebas{A word about what problem phizura help us fix)}
\end{abstract}
...
\sebas{We can say that we can use phizura to replay livecoding performances and do cool stuff}.
...
\sebas{A word about future work}.

\section{Background}\label{sec:bac}

\subsection{Live coding}

\sebas{Pienso que es importante que empecemos hablando de live coding. Que el enfoque del paper sea más del lado de live coding y que el público sea informáticos que no conoces mucho sobre live coding.}
...
The keystroke capture partially addresses this gap by logging individual key events with timestamps and cursor positions, but it does not reconstruct the full editing history in a way that could be replayed as a visual typing sequence. That would require a different approach, closer to what Live Writing~\cite{Lee15a} provides.\sebas{I dont inderstand why the keystores and timestramps are not enough,}

Timing fidelity depends on the granularity of \texttt{DateAndTime now} and the accuracy of \texttt{seconds wait} during replay.\sebas{Actually we capture the time in milliseconds using a native method.} For musical purposes, where consecutive evaluations are typically separated by one to several seconds, this is more than sufficient: the human performer's own timing variability is far larger than any clock imprecision.

Finally, the current design supports a single recording session at a time, managed through a global variable. Collaborative live coding scenarios, where multiple performers evaluate code in separate Playgrounds, would require extending the architecture to support multiple concurrent recorders.\sebas{We could have one phizura instance per playground and no longer have the singleton.}
