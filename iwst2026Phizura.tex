...
Phizura is a music recording for Copyu that captures every code evaluation and produces executable code that replay the music with the same delays.
Its redesign starts from a single observation: every expression a performer evaluates already passes through a small number of well-defined methods inside the Playground.
Method Proxies can intercept those methods from the outside, which means the recorder never needs to live inside Coypu.
